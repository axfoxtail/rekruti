...
@Component({
  selector: 'app-companies-sidebar',
  templateUrl: './companies-sidebar.component.html',
  styleUrls: ['./companies-sidebar.component.css']
})
export class CompaniesSidebarComponent implements OnInit {

  pageTitle: any = 'Companies';
  filtersList: any;
  companiesData: any;
  ...
  selectBucket(bucket: any, filter: any) {
    const body = this.search.selectBucket(bucket, filter, this.globalVar.getSearchConditionsCompanies());
  ...
  addBucket(filter: any) {
    const body = this.search.addBucket(this.addFilterOption.value.addFacetsObject, filter, this.globalVar.getSearchConditionsCompanies());
    this.globalVar.companiesListChanged(body);
  }

  addBucketKeyword(filter: any) {
    const body = this.search.addBucketKeyword(this.addFilterOption.value.keywords, filter, this.globalVar.getSearchConditionsCompanies());
    this.globalVar.companiesListChanged(body);
  }
